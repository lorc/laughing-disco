    ...
    def handle_and_or(e: BoolExpression, state: MatchState) -> MatchState:
        ...
        if not isinstance(e.b, BoolExpression) or op_b_is_implicit_cast:
            ...
            match instructions[new_state.instr_idx].mnemonic:
                ...
                case mnemonic:
                    if mnemonic == "b":
                        prev = new_state.instr_idx - 1
                        if prev < 0 or instructions[prev].mnemonic not in ("str", "stur") \
                           or not reg_cmp(get_instr_reg_operand(instructions[prev], 0),
                                          new_state.target_reg):
                            raise MatchError(
                                f"Expected tb(n)z, cb(n)z, b.eq, b.ne but found {mnemonic}")
                        ret.append(TracePoint(instructions[prev].address, False, e.b))
        ...
        match instructions[new_state.instr_idx].mnemonic:
            ...
            case "tbnz":
                is_unsigned = "unsigned" in str(e.a).lower() \
                               and not (isinstance(e.b, IntLiteral) and e.b.value == 0)
